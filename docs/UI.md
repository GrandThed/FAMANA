...
- **Inventory:** window `x=352, y=30, w=770, h=640` plus a traits column (a SpellTrackerUI instance) on its left. Paperdoll column 308px (avatar center as a `ViewportFrame` — see §10), 12 equipment slots around it; the 10-wide grid fills the right. Tooltip floats over the grid anchored to the hovered item. The trait rail ALSO exists as a second always-on instance in the top-right menu stack (`TopRightMenu`), directly under the Inventory/Character/Craft buttons, hover popouts opening leftward.
- **HUD:** compact rail top-left (`x=20,y=20`); target plate top-center; the top-right menu stack (`TopRightMenu`: gear + Inventory/Character/Craft buttons, trait rail below) inset 16px from the corner; HP orb bottom-left, mana orb bottom-right; hotbar + XP bar bottom-center.
