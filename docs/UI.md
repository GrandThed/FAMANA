...
- **Inventory:** window `x=352, y=30, w=770, h=640`. Paperdoll column 308px on the left (avatar center as a `ViewportFrame` — see §10), 12 equipment slots around it; the 10-wide grid fills the right. Tooltip floats over the grid anchored to the hovered item. The trait rail (the mock's left rail) lives on the RIGHT screen edge as an always-on strip, vertically centered at 0.42; its hover popouts open leftward.
